...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/zzsh/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'c1', title: '主题', align: 'left', width: 150},

                {field: 'c2', title: '时间', align: 'left', width: 150},

                {field: 'c3', title: '地点', align: 'left', width: 150},

                {field: 'c4', title: '主持人', align: 'left', width: 150},

                {field: 'c5', title: '记录人', align: 'left', width: 150},

                {field: 'c6', title: '到会指导干部', align: 'left', width: 150},

                {field: 'c7', title: '应到人数', align: 'left', width: 150},

                {field: 'c8', title: '实到人数', align: 'left', width: 150},

                {field: 'c9', title: '主要内容', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/zzsh/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/zzsh/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/zzsh/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
