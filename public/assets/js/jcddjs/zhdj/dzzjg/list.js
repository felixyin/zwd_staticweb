...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/dzzjg/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'c1', title: '基层党组织（党支部）名称', align: 'left', width: 150},

                {field: 'c2', title: '书记', align: 'left', width: 150},

                {field: 'c3', title: '副书记', align: 'left', width: 150},

                {field: 'c4', title: '组织委员', align: 'left', width: 150},

                {field: 'c5', title: '宣传委员', align: 'left', width: 150},

                {field: 'c6', title: '纪检委员', align: 'left', width: 150},

                {field: 'c7', title: '统战委员', align: 'left', width: 150},

                {field: 'c8', title: '党员人数', align: 'left', width: 150},

                {field: 'c9', title: '党员小组数', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/dzzjg/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/dzzjg/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/jcddjs/zhdj/dzzjg/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
