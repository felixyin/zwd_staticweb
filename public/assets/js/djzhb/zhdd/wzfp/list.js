...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/zhdd/wzfp/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'addr', title: '房屋地址', align: 'left', width: 150},

                {field: 'area', title: '面积', align: 'left', width: 150},

                {field: 'username', title: '承租人', align: 'left', width: 150},

                {field: 'yongtu', title: '用途', align: 'left', width: 150},

                {field: 'begintime', title: '承租日期', align: 'left', width: 150},

                {field: 'endtime', title: '到期日期', align: 'left', width: 150},

                {field: 'money', title: '租金', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/zhdd/wzfp/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#area_edit").val(num.area);

                $("#username_edit").val(num.username);

                $("#yongtu_edit").val(num.yongtu);

                $("#begintime_edit").val(num.begintime);

                $("#endtime_edit").val(num.endtime);

                $("#money_edit").val(num.money);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/zhdd/wzfp/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/zhdd/wzfp/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
