...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/dagl/dyda/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'branch', title: '所属机构', align: 'left', width: 150},

                {field: 'username', title: '姓名', align: 'left', width: 150},

                {field: 'idcard', title: '身份证', align: 'left', width: 150},

                {field: 'pic', title: 'url,url,', align: 'left', width: 150},

                {field: 'type', title: '党员类别', align: 'left', width: 150},

                {field: 'viwu', title: '职务', align: 'left', width: 150},

                {field: 'files', title: 'f，f,', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/dagl/dyda/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#branch_edit").val(num.branch);

                $("#username_edit").val(num.username);

                $("#idcard_edit").val(num.idcard);

                $("#pic_edit").val(num.pic);

                $("#type_edit").val(num.type);

                $("#viwu_edit").val(num.viwu);

                $("#files_edit").val(num.files);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/dagl/dyda/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/dagl/dyda/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
