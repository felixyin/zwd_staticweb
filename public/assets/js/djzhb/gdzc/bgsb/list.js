...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/gdzc/bgsb/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'devname', title: '设备名称', align: 'left', width: 150},

                {field: 'addr', title: '存放位置', align: 'left', width: 150},

                {field: 'dept', title: '使用部门', align: 'left', width: 150},

                {field: 'username', title: '使用人', align: 'left', width: 150},

                {field: 'fenpeitime', title: '分配日期', align: 'left', width: 150},

                {field: 'nianxian', title: '折旧年限', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/gdzc/bgsb/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#devname_edit").val(num.devname);

                $("#addr_edit").val(num.addr);

                $("#dept_edit").val(num.dept);

                $("#username_edit").val(num.username);

                $("#fenpeitime_edit").val(num.fenpeitime);

                $("#nianxian_edit").val(num.nianxian);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/gdzc/bgsb/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/gdzc/bgsb/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
