...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/gcwx/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'car_id', title: '车id', align: 'left', width: 150},

                {field: 'username', title: '负责人', align: 'left', width: 150},

                {field: 'no', title: '车牌号', align: 'left', width: 150},

                {field: 'type', title: '车型', align: 'left', width: 150},

                {field: 'pic', title: '照片', align: 'left', width: 150},

                {field: 'addr', title: '维修地点', align: 'left', width: 150},

                {field: 'fee', title: '费用', align: 'left', width: 150},

                {field: 'dtime', title: '维修时间', align: 'left', width: 150},

                {field: 'desc', title: '备注', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/gcwx/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#car_id_edit").val(num.car_id);

                $("#username_edit").val(num.username);

                $("#no_edit").val(num.no);

                $("#type_edit").val(num.type);

                $("#pic_edit").val(num.pic);

                $("#addr_edit").val(num.addr);

                $("#fee_edit").val(num.fee);

                $("#dtime_edit").val(num.dtime);

                $("#desc_edit").val(num.desc);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/gcwx/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/gcwx/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
