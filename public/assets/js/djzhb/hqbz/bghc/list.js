...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/bghc/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'categ', title: '分类', align: 'left', width: 150},

                {field: 'name', title: '名称', align: 'left', width: 150},

                {field: 'supl', title: '供应商', align: 'left', width: 150},

                {field: 'kucun', title: '库存', align: 'left', width: 150},

                {field: 'dtime', title: '采购时间', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/bghc/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#categ_edit").val(num.categ);

                $("#name_edit").val(num.name);

                $("#supl_edit").val(num.supl);

                $("#kucun_edit").val(num.kucun);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/bghc/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/hqbz/bghc/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
