...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/fwdw/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'fwh', title: '发文号', align: 'left', width: 150},

                {field: 'dtime', title: '发文日期', align: 'left', width: 150},

                {field: 'filename', title: '文件名称', align: 'left', width: 150},

                {field: 'zsdw', title: '祝颂单位', align: 'left', width: 150},

                {field: 'nwbm', title: '拟文部门', align: 'left', width: 150},

                {field: 'nwqz', title: '拟文人签字', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/fwdw/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#fwh_edit").val(num.fwh);

                $("#dtime_edit").val(num.dtime);

                $("#filename_edit").val(num.filename);

                $("#zsdw_edit").val(num.zsdw);

                $("#nwbm_edit").val(num.nwbm);

                $("#nwqz_edit").val(num.nwqz);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/fwdw/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/fwdw/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
