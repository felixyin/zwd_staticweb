...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/swzf/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'dtime', title: '来文日期', align: 'left', width: 150},

                {field: 'lwzh', title: '来文字号', align: 'left', width: 150},

                {field: 'lwdw', title: '来文单位', align: 'left', width: 150},

                {field: 'pwsx', title: '来文份数', align: 'left', width: 150},

                {field: 'rjh', title: '入卷号', align: 'left', width: 150},

                {field: 'wjzy', title: '文件摘要', align: 'left', width: 150},

                {field: 'nbyj', title: '拟办意见', align: 'left', width: 150},

                {field: 'nbsj', title: '拟办时间', align: 'left', width: 150},

                {field: 'cyyj', title: '传阅时间', align: 'left', width: 150},

                {field: 'ssbm', title: '所涉部门', align: 'left', width: 150},

                {field: 'lsqk', title: '落实情况', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/swzf/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#dtime_edit").val(num.dtime);

                $("#lwzh_edit").val(num.lwzh);

                $("#lwdw_edit").val(num.lwdw);

                $("#pwsx_edit").val(num.pwsx);

                $("#rjh_edit").val(num.rjh);

                $("#wjzy_edit").val(num.wjzy);

                $("#nbyj_edit").val(num.nbyj);

                $("#nbsj_edit").val(num.nbsj);

                $("#cyyj_edit").val(num.cyyj);

                $("#ssbm_edit").val(num.ssbm);

                $("#lsqk_edit").val(num.lsqk);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/swzf/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/sfw/swzf/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
