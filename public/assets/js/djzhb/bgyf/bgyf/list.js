...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/bgyf/bgyf/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'no', title: '序号', align: 'left', width: 150},

                {field: 'dwmc', title: '单位名称', align: 'left', width: 150},

                {field: 'fjh', title: '房间号', align: 'left', width: 150},

                {field: 'pm', title: '平米', align: 'left', width: 150},

                {field: 'xm', title: '姓名', align: 'left', width: 150},

                {field: 'zw', title: '职务', align: 'left', width: 150},

                {field: 'bz', title: '备注', align: 'left', width: 150},

                {field: 'dwf', title: '单位负责人', align: 'left', width: 150},

                {field: 'jjf', title: '纪检负责人', align: 'left', width: 150},

                {field: 'lxdh', title: '联系电话', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/bgyf/bgyf/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#dwmc_edit").val(num.dwmc);

                $("#fjh_edit").val(num.fjh);
                ...
                $("#dwf_edit").val(num.dwf);

                $("#jjf_edit").val(num.jjf);

                $("#lxdh_edit").val(num.lxdh);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/bgyf/bgyf/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/bgyf/bgyf/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
