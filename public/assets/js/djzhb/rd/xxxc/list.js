...
$(function () {
    ...
    layui.use(['table', 'form', 'laypage', 'layer', 'element', 'laydate', 'upload'], function () {
        ...
        table.render({
            ...
            method: 'get',
            loading: true,
            url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/rd/xxxc/list.json'+'?t='+(new Date().getTime()),//数据接口
            headers: setHeader(),
            ...
            cols: [[    //表头
                ...
                {field: 'name', title: '代表姓名', align: 'left', width: 150},

                {field: 'hddd', title: '活动地点', align: 'left', width: 150},

                {field: 'hdsj', title: '活动时间', align: 'left', width: 150},

                {field: 'hdzt', title: '活动主题', align: 'left', width: 150},

                {field: 'hdrs', title: '活动人数', align: 'left', width: 150},

                {field: 'hdxs', title: '活动形式', align: 'left', width: 150},

                {field: 'hdnr', title: '活动内容', align: 'left', width: 150},

                {field: 'type', title: '类型', align: 'left', width: 150},

                {field: 'hdkqb', title: '活动考勤表附件', align: 'left', width: 150},

                {field: 'bssdjb', title: '办实事登记表附件', align: 'left', width: 150},
        ...
        table.on('tool(test)', function (obj) {
            ...
            if (obj.event === 'del') {  //删除
                layer.confirm('真的要删除这行数据？', function (index) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/assets/js/api/data/djzhb/rd/xxxc/remove.json'+'?t='+(new Date().getTime()),//数据接口
                        type: "POST",              //请求类型
                ...
                $("#hddd_edit").val(num.hddd);

                $("#hdsj_edit").val(num.hdsj);

                $("#hdzt_edit").val(num.hdzt);

                $("#hdrs_edit").val(num.hdrs);

                $("#hdxs_edit").val(num.hdxs);

                $("#hdnr_edit").val(num.hdnr);

                $("#type_edit").val(num.type);

                $("#hdkqb_edit").val(num.hdkqb);

                $("#bssdjb_edit").val(num.bssdjb);
        ...
        form.on('submit(addInfo)', function (data) {
            ...
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/rd/xxxc/add.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
        ...
        form.on('submit(editInfo)', function (data) {
            ...
            var requestData = formToJsonObj("form_edit", []);
            $.ajax({
                url:'http://127.0.0.1:8000/assets/js/api/data/djzhb/rd/xxxc/edit.json'+'?t='+(new Date().getTime()),//数据接口
                type: "POST",              //请求类型
                data: formatParam(requestData),              //请求数据
                timeout: 200000,                //请求超时时间(毫秒)
                headers: setHeader(),
